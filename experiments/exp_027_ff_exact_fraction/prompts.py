...
exp_027 single change vs exp_024: SYSTEM_PROMPT_MATH gains an exact-fraction /
no-round instruction (free-form only). MCQ prompt + few-shots are byte-identical
to exp_024. Hypothesis: the wrong_math PRECISION bucket (~50 on full public —
right values judged wrong at the judger's 1e-08 tolerance because the model
rounded to 2–4 digits) is recoverable if the model emits an exact fraction or
>=10 significant figures. Dev-only probe; watch MCQ for the exp_005 regression.
"""
...
SYSTEM_PROMPT_MATH = (
    ...
    "(for example \\boxed{a/b}) instead of a decimal. If an answer is irrational or can only be "
    "given as a decimal, write at least 10 significant figures and do not round to fewer digits."
)
