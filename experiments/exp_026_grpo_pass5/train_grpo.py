"""GRPO pass 4 training script — DSMLP batch-mode friendly.

ONE intentional change from exp_019 (pass 3): the curriculum's per-token sampling
distribution now matches training. Every prior GRPO pass sampled difficulty at
vLLM top_k=20 / top_p=0.95 but trained at TRL/transformers top_k=None / top_p=1.0
(truncation disabled — confirmed from TRL 0.21 source). That distribution gap
was baked into every curriculum we ever used: a prompt at 2/4 correct under
top_k=20 could be 4/4 or 0/4 under the training distribution, producing dead
steps independent of G or T. pass-4's curriculum_pass4.json is sampled from the
pass-3 policy at training-matched top_k=-1 / top_p=1.0 / T=1.0 / N=8.

Everything else byte-identical to exp_019:
  - base_model = TrevorDuong/qwen3-4b-thinking-grpo-pass3 (the freshly-merged pass-3)
  - num_generations=4, temperature=1.0, max_completion_length=5120
  - lr=2e-5, beta=0.01, length_bonus(max=0.05, cap=16384)
  - pdbs=1, grad_accum=4, lora_r=16, lora_alpha=32, lora_dropout=0.0

Why we did NOT bump G or T: the pass-4 pilot at PILOT_STEPS=6 was too noisy to
discriminate (frac_reward_zero_std=0 across all configs is a sample-size
artifact). The plain read was G=6 actively *hurt* correctness variance vs G=4;
T=1.1 only recovered it. Rather than gamble on speculative levers, we change
the one thing that's an unambiguous upstream fix: the curriculum.

One safety fix vs exp_019: MERGED_DIR points to /tmp/merged_final (ephemeral),
not the experiment dir on the 5GB home PVC. The inline fp16 merge crashed both
exp_015 and exp_019 on PVC quota; HF Hub upload still happens, only the local
copy moves.
...
container's _try_resume_from_hf() picks up where it left off.
...
# bug-082 prevention: write the 8GB fp16 merge to /tmp (ephemeral) instead of the
# 5GB home PVC. exp_015 and exp_019 each crashed mid-merge on the PVC quota; the
# merge is pushed straight to HF Hub and the local copy is throwaway, so /tmp is
# the right home. /tmp on the scipy-ml-notebook pod has tens of GB free.
MERGED_DIR  = Path("/tmp") / "merged_final"
