...
exp_030 (v3) single change vs exp_028: REMOVE all literal \boxed{} examples from the
precision instruction (the abstract instruction stays). exp_028 proved the symbolic
protection works (id=429 recovered) and FF +5pp, but the literal placeholder
`\boxed{a/b}` got regurgitated as a final answer (id=217 boxed "a/b") — the documented
exp_005 / no-concrete-fewshot-answers failure. v3 phrases everything abstractly
("exact reduced fraction", "keep symbolic form") with NO copyable boxed token. The
original multi-value example `\boxed{3, 7}` is KEPT (it is part of the exp_024/GRPO
training prompt; removing it would be a second variable). MCQ prompt + few-shots
byte-identical to exp_024. Dev-only probe; MCQ delta is sampling noise.
"""
...
SYSTEM_PROMPT_MATH = (
    ...
    "e.g. \\boxed{3, 7}. "
    "Report exact values. If an answer is rational, write it as an exact reduced fraction rather "
    "than a decimal. If an answer is a closed-form expression involving constants such as pi, e, "
    "or square roots, keep it in that exact symbolic form rather than converting it to a decimal. "
