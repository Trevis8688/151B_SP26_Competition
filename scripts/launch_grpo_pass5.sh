#!/bin/bash
# DSMLP launch wrapper for exp_026 GRPO pass 5 training.
# Byte-for-byte clone of launch_grpo_pass4.sh except EXP. Same recipe (G=4, T=1.0,
# max_completion=5120). The intentional pass-5 change is base_model + CURRICULUM
# advancing one pass (pass-4 -> pass-5), not the training stack — see
# experiments/exp_026_grpo_pass5/config.json _comment.
#
# Prereq: experiments/exp_026_grpo_pass5/curriculum_pass5.json must exist. Build
# it from data/difficulty_samples_pass4.jsonl with filter_curriculum_v2.py — see
# the command at the top of launch_difficulty_pass4.sh.
#
# Run from the dsmlp-login node:
#   bash scripts/launch_grpo_pass5.sh
#
...
if [ ! -f "experiments/$EXP/curriculum_pass5.json" ]; then
  echo "ERROR: experiments/$EXP/curriculum_pass5.json missing." >&2
  echo "Build it first: " >&2
  echo "  python scripts/filter_curriculum_v2.py \\" >&2
  echo "    --in data/difficulty_samples_pass4.jsonl \\" >&2
  echo "    --out experiments/$EXP/curriculum_pass5.json \\" >&2
  echo "    --min-correct 2 --max-correct 6 --allow-clipped --ff-mcq-ratio 2.0" >&2
  exit 1
fi

echo "Launching GRPO pass 5 on $GPU ..."
