...
K8S_TIMEOUT_SECONDS=43200 launch.sh \
  ...
  -- bash -c '
    ...
    # ---- pass-4 model, sampler matched to a top_k-disabled T=1.0 training, at the
    #      5120 training budget. LIMIT is env-overridable from the OUTER shell:
    #      `LIMIT=20 bash scripts/launch_difficulty_pass4.sh` smokes 20 prompts;
    #      bare `bash ...` (LIMIT=0) runs the full 1126. ----
    LIMIT='"${LIMIT:-0}"' \
    MODEL_ID="TrevorDuong/qwen3-4b-thinking-grpo-pass4" \
    OUT_SUFFIX="pass4" \
    NUM_SAMPLES=8 \
    TEMPERATURE=1.0 \
    TOP_K=-1 \
...
echo ""
echo "Pod launched (FULL RUN, pass-4 model, N=8, top_k disabled)."
echo "Find name with: kubectl get pods"
echo "Tail logs with: kubectl logs -f <pod_name>"
echo ""
echo "Outputs (in PVC, persist across pods):"
echo "  ~/151B_SP26_Competition/data/difficulty_samples_pass4.jsonl"
echo "  ~/151B_SP26_Competition/data/sweet_spot_pass4_ids.json"
echo ""
echo "Expected runtime: ~4-8h (N=8 is 2x the pass-2 N=4 run). First chunk in ~10-15 min."
echo "SMOKE TEST FIRST:  LIMIT=20 bash scripts/launch_difficulty_pass4.sh"
echo ""
echo "Then build the pass-5 curriculum WITHOUT re-sampling:"
echo "  python scripts/filter_curriculum_v2.py \\"
echo "    --in data/difficulty_samples_pass4.jsonl \\"
echo "    --out experiments/exp_026_grpo_pass5/curriculum_pass5.json \\"
